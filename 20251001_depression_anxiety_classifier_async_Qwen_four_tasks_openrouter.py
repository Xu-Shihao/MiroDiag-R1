...
def parse_args():
    ...
    parser.add_argument("--input", type=str, default="./data/validation_data.xlsx", 
                        help="输入Excel文件路径")
    parser.add_argument("--output", type=str, default="./results/test.xlsx", 
                        help="输出Excel文件路径")
...
async def log_request_response(visit_number, prompt, response, model_name, classification_mode, ground_truth=None, save_json_logs=True):
    """记录请求和响应到JSON日志"""
...
def save_json_logs_to_file(output_prefix, model_name, classification_mode):
    """保存JSON日志到文件"""
...
def find_existing_json_logs(output_prefix, model_name, classification_mode):
    """查找已存在的JSON日志文件"""
    try:
        import glob
        
        model_basename = os.path.basename(model_name.rstrip('/'))
        pattern = f"./results/*_{model_basename}_{classification_mode}_*_logs.json"
...
def create_prompt(text, classification_mode="binary"):
    if classification_mode == "binary":
        return rf"""你是一位经验丰富的精神科医生。请阅读以下病人来精神科问诊的对话记录，并判断病人更可能患有抑郁症还是焦虑症。
...
请用中文一步一步思考，之后将"抑郁"或者"焦虑"的结果放在<box>xxx</box>中输出。"""
        ...
        return rf"""你是一位经验丰富的精神科医生。请阅读以下病人来精神科问诊的对话记录，并判断病人的主要心理健康状况。抑郁症以**持续的情绪低落、兴趣减退和精力缺乏**为主，而焦虑症则以**过度担忧、紧张不安和对未来事件的恐惧**为主要特点。
...
请用中文一步一步思考，之后将结果（"抑郁"、"焦虑"、"mix"或"others"）放在<box>xxx</box>中输出。"""
        ...
        return rf"""你是一位经验丰富的精神科医生。请阅读以下精神科问诊的对话记录，并判断对话的人是否主要表现为抑郁症, 满足ICD诊断要求。
...
请用中文一步一步思考，之后判断问诊的人是否是抑郁症，将"是"或"否"放在<box>xxx</box>中输出。"""
        ...
        return rf"""你是一位经验丰富的精神科医生。请阅读以下精神科问诊的对话记录，并判断对话的人是否主要表现为焦虑症, 满足ICD诊断要求。
...
请用中文一步一步思考，之后判断病人是否有焦虑症，将"是"或"否"放在<box>xxx</box>中输出。"""
        ...
        return rf"""你是一位经验丰富的精神科医生。请阅读以下精神科问诊的对话记录，并根据ICD-10标准单独判断对话的人是否患有抑郁症和焦虑症。
...
请用中文一步一步思考，之后将结果放在<box>xxx</box>中输出。"""
        ...
        return rf"""你是一位经验丰富的精神科医生。请阅读以下精神科问诊的对话记录，并根据ICD-10国际疾病分类标准，给出最符合患者状况的具体诊断代码。
...
请用中文一步一步思考，之后直接给出最符合的ICD-10诊断代码，格式为"Fxx.x"，放在<box>xxx</box>中输出。"""
        ...
        return rf"""你是一位经验丰富的精神科医生。请阅读以下初次精神科门诊的问诊对话记录，并根据ICD-10国际疾病分类标准，仔细分析后输出患者诊断结束后的ICD-10诊断代码。

[问诊对话开始]
{text}
[问诊对话结束]

请仅从以下10种疾病中选择最符合的诊断大类以及进一步细分的小类：
    - F32 抑郁发作：情绪持续低落、兴趣/愉快感下降、精力不足；伴睡眠/食欲改变、自责/无价值感等；可轻/中/重度（重度可伴精神病性症状）；无既往躁狂/轻躁狂。
    - F41 其他焦虑障碍：恐慌发作或广泛性焦虑为主；过度担忧、紧张、心悸、胸闷、出汗、眩晕、濒死感/失控感；与特定情境无关或不成比例，造成显著痛苦/功能损害。
    - F39 未特指的心境（情感）障碍：存在心境障碍证据，但资料不足以明确归入抑郁或双相等具体亚型时选用。
    - F51 非器质性睡眠障碍：失眠、过度嗜睡、梦魇、昼夜节律紊乱等；非器质性原因；睡眠问题为主要主诉并致显著困扰/功能损害。
...
注意：
1. 初次问诊，在症状严重程度不可判断的时候，请推荐unspecified的code(e.g.,F32.9, F41.2, F39.x等)。
2. 诊断结果可能包含1至3个icd-10诊断结果，大多只包含一个但推荐不超过3个。
3. 用分号分隔不同的代码。
4. 需要严格根据icd-10标准来, 避免猜测和无根据的诊断，保证诊断错误。
5. 优先考虑上述11类，但如果症状明确指向其他ICD-10代码，也可以诊断。
...
async def get_token_probabilities_async(client, semaphore, prompt, model_name, max_retries=3, top_logprobs=20, site_url="", site_name="", api_type="openrouter"):
...
def extract_icd10_code(response, debug=False):
    ...
    # 标准化ICD-10代码格式
    # 匹配F开头的代码，如F32, F32.1, F41.0等
    icd10_pattern = r'F\d+(?:\.\d+)?'
...
def preprocess_diagnosis_codes(diagnosis_code):
    ...
    import re
    # 使用正则表达式匹配F开头的ICD代码
    codes = re.findall(r'F\d+(?:\.\d+)?', code_str, re.IGNORECASE)
...
async def process_record_async(client, semaphore, visit_number, text, model_name, classification_mode="binary", primary_diagnosis=None, debug=False, save_full_response=True, top_logprobs=20, site_url="", site_name="", save_json_logs=True, api_type="openrouter"):
    ...
    # 创建prompt
    prompt = create_prompt(text, classification_mode)
    
    # 获取token概率
    response = await get_token_probabilities_async(client, semaphore, prompt, model_name, top_logprobs=top_logprobs, site_url=site_url, site_name=site_name, api_type=api_type)
    
    # 记录请求和响应到JSON日志
    await log_request_response(visit_number, prompt, response, model_name, classification_mode, primary_diagnosis, save_json_logs)
...
# 可视化结果
def visualize_results(results_df, output_prefix, classification_mode="binary"):
    """生成结果可视化图表"""
    try:
        # 确保输出目录存在
        ...
        if classification_mode == "binary":
            ...
            try:
                ...
                plt.tight_layout()
                plt.savefig(f"results_visualization/{output_prefix}_probability_distributions.png")
                plt.close()  # 释放图表资源
            ...
            try:
                ...
                plt.savefig(f"results_visualization/{output_prefix}_depression_vs_anxiety.png")
                plt.close()  # 释放图表资源
                logger.info("散点图已保存")
            except Exception as e:
            ...
            try:
                ...
                if not (math.isnan(avg_depression) or math.isnan(avg_anxiety)):
                    ...
                    plt.savefig(f"results_visualization/{output_prefix}_overall_distribution.png")
                    logger.info("饼图已保存")
                else:
            ...
            try:
                ...
                plt.tight_layout()
                plt.savefig(f"results_visualization/{output_prefix}_multiclass_distributions.png")
                plt.close()  # 释放图表资源
            ...
            try:
                ...
                if values:
                    ...
                    plt.savefig(f"results_visualization/{output_prefix}_multiclass_distribution.png")
            ...
            try:
                ...
                plt.tight_layout()
                plt.savefig(f"results_visualization/{output_prefix}_depression_symptom_distributions.png")
                plt.close()
            ...
            try:
                ...
                if not (math.isnan(avg_has_symptom) or math.isnan(avg_no_symptom)):
                    ...
                    plt.savefig(f"results_visualization/{output_prefix}_depression_symptom_pie.png")
            ...
            try:
                ...
                plt.tight_layout()
                plt.savefig(f"results_visualization/{output_prefix}_anxiety_symptom_distributions.png")
                plt.close()
            ...
            try:
                ...
                if not (math.isnan(avg_has_symptom) or math.isnan(avg_no_symptom)):
                    ...
                    plt.savefig(f"results_visualization/{output_prefix}_anxiety_symptom_pie.png")
        ...
        logger.info(f"结果可视化图表已保存在 results_visualization 目录下")
...
# 可视化评估结果
def visualize_evaluation_results(evaluation_metrics, output_prefix):
    """可视化评估结果，包括混淆矩阵和指标对比"""
    ...
    plt.tight_layout()
    plt.savefig(f"results_visualization/{output_prefix}_evaluation_results.png", 
                dpi=300, bbox_inches='tight')
    plt.close()
    
    logger.info(f"评估结果可视化图表已保存至: results_visualization/{output_prefix}_evaluation_results.png")
...
async def main():
    ...
    log_filename = f"./logs/{current_date}_{model_basename}_{args.classification_mode}_{current_time}.log"
    ...
    if args.evaluate_only:
        ...
        try:
            ...
            if evaluation_metrics:
                ...
                visualize_evaluation_results(evaluation_metrics, output_prefix)
    ...
    # Resume功能：检查是否存在之前的JSON日志文件
    output_prefix = os.path.splitext(os.path.basename(OUTPUT_FILE))[0]
    existing_json_file, previous_json_data = find_existing_json_logs(output_prefix, MODEL_NAME.replace("/", "_"), args.classification_mode)
    ...
    if existing_json_file and previous_json_data:
        ...
        if len(additional_sampled_df) == 0:
            ...
            if args.evaluate and len(results_df) > 0:
                logger.info("=== 开始模型评估 ===")
                logger.info(f"用于评估的样本数: {len(results_df)}")
                evaluation_metrics = calculate_evaluation_metrics(results_df, args.classification_mode)
                
                if evaluation_metrics:
                    logger.info("评估完成，正在生成可视化图表...")
                    visualize_evaluation_results(evaluation_metrics, output_prefix)
                else:
                    logger.warning("评估指标计算失败")
    ...
    new_results_list = []
    if len(tasks) > 0:
        for future in tqdm(asyncio.as_completed(tasks), total=len(tasks), desc="分析文本"):
            result = await future
            new_results_list.append(result)
        
        # 将新结果转换为DataFrame
        new_results_df = pd.DataFrame(new_results_list)
        logger.info(f"新处理了 {len(new_results_df)} 个样本")
    else:
    ...
    try:
        ...
        if args.save_json_logs:
            output_prefix = os.path.splitext(os.path.basename(output_file))[0]
            json_log_file = save_json_logs_to_file(output_prefix, MODEL_NAME, args.classification_mode)
            if json_log_file:
                logger.info(f"请求响应日志已保存至: {json_log_file}")
