...
INPUT_FILE="./data/SMHC_${method_name}_validation_data.xlsx"
SCRIPT_NAME="20251003_depression_anxiety_classifier_async_Qwen_four_tasks_openrouter.py"
OUTPUT_DIR="./results/${method_name}/results"
LOGS_DIR="./results/${method_name}/logs"
RESULTS_DIR="./results/${method_name}/results"
...
if [ "$CONFIG_TYPE" != "custom" ] && declare -f select_test_config > /dev/null; then
    ...
    # 基础配置
    LIMIT=200                   # 每个测试的样本数量
    MAX_CONCURRENT=16           # 最大并发数（免费模型建议2，付费模型可以更高）
